...
export const ModuleForm: React.FC<FormProps> = ({ data, onChange }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md space-y-4 border border-teal-100">
      <div className="flex justify-between items-center border-b pb-2 mb-4">
        <h2 className="text-xl font-bold text-teal-800 flex items-center gap-2">
          <span className="bg-teal-600 text-white p-1 rounded">📝</span> Input Data Modul
        </h2>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-600 uppercase">Jenis Kokurikuler</label>
          <select 
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-600 uppercase">Alokasi Waktu (JP)</label>
          ...
          <p className="text-[10px] text-teal-600 italic">Asumsi MI: 108 JP/Tahun, 54 JP/Semester</p>
        </div>
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-600 uppercase">Nama Kegiatan / Tema</label>
          <select 
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-600 uppercase">Tahun Pelajaran</label>
          <input 
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-600 uppercase">Input Titimangsa</label>
          <input 
      ...
      <div className="space-y-1">
        <div className="flex justify-between items-center">
          <label className="text-xs font-bold text-gray-600 uppercase">Karakteristik Peserta Didik</label>
          <button 
            onClick={handleGenerateKarakteristik}
            disabled={loadingKarakteristik}
            className="text-[10px] bg-purple-100 text-purple-700 px-2 py-1 rounded hover:bg-purple-200 transition-colors flex items-center gap-1 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-600 uppercase block">Dimensi Profil Lulusan (Boleh Pilih Lebih dari Satu)</label>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 border border-gray-200 rounded-lg bg-gray-50">
            {dimensiOptions.map(option => (
              <label key={option} className="flex items-center gap-2 text-sm cursor-pointer hover:text-teal-700 group">
                <input 
                  type="checkbox"
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-gray-600 uppercase block">Topik Panca Cinta (Boleh Pilih Lebih dari Satu)</label>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 border border-gray-200 rounded-lg bg-gray-50">
      ...
      <div className="space-y-1">
        <label className="text-xs font-bold text-gray-600 uppercase">Tujuan Pembelajaran</label>
        <textarea 
      ...
      <div className="space-y-1">
        <label className="text-xs font-bold text-gray-600 uppercase">Praktik Pedagogis</label>
        <select 
      ...
      {data.deskripsi_kegiatan_ai !== undefined && (
        <div className="border-t border-gray-200 pt-4 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-500 uppercase">Deskripsi Kegiatan</label>
            <textarea 
              name="deskripsi_kegiatan_ai" value={data.deskripsi_kegiatan_ai || ''} onChange={handleChange}
              rows={4} className="w-full p-2 border border-gray-300 rounded text-sm"
            />
          </div>
          <div className="grid grid-cols-1 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-gray-500 uppercase">Tahap 1: Memahami</label>
              <textarea name="langkah_tahap_1" value={data.langkah_tahap_1 || ''} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded text-sm" />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-gray-500 uppercase">Tahap 2: Mengaplikasi</label>
              <textarea name="langkah_tahap_2" value={data.langkah_tahap_2 || ''} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded text-sm" />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-gray-500 uppercase">Tahap 3: Merefleksi</label>
              <textarea name="langkah_tahap_3" value={data.langkah_tahap_3 || ''} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded text-sm" />
            </div>
          </div>
        </div>
      )}
