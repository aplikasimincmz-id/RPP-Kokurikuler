...
export const ModulePreview: React.FC<PreviewProps> = ({ data, onEdit, pdfSections, showPageNumbers }) => {
  ...
  const renderSection = (id: string, index: number) => {
    switch(id) {
      ...
      case 'identifikasi':
        return (
          <section key={id} id="section-1" className="mb-6">
            {getSectionHeader(1)}
      ...
      case 'desainPembelajaran':
        return (
          <section key={id} id="section-2" className="mb-6">
            {getSectionHeader(2)}
            <div className="pl-4 mt-2 space-y-4 text-sm">
              <table className="w-full border-none">
                <tbody>
      ...
      case 'deskripsiKegiatan':
        return (
          <section key={id} id="section-3" className="mb-6">
            {getSectionHeader(3)}
      ...
      case 'langkahKegiatan':
        return (
          <section key={id} id="section-4" className="mb-6">
            {getSectionHeader(4)}
      ...
      case 'asesmen':
        return (
          <section key={id} id="section-5" className="mb-6 page-break-before">
            {getSectionHeader(5)}
      ...
      case 'lampiran':
        return (
          <section key={id} id="appendix-section" className="mt-10 border-t-2 border-dashed border-gray-300 dark:border-neutral-700 pt-8 page-break-before transition-colors">
            <h2 className="bg-primary-900 text-white px-2 py-1 font-bold text-base mb-4 flex justify-between transition-colors">
              ...
              <button 
                onClick={() => onEdit(5)}
                className="print:hidden ml-2 bg-white/20 hover:bg-white/30 text-white px-2 py-0.5 rounded flex items-center gap-1 transition-colors"
              >
                <span className="font-normal text-[8px] uppercase tracking-wider">Perbaiki</span>
              </button>
            </h2>
            ...
            <div className="page-break-before">
              ...
              <table className="w-full border-collapse border-2 border-black text-[11px] lampiran-table">
                <tbody>
                  <tr>
                    <td className="border border-black p-3 bg-gray-50/50 font-bold w-40">Objek Observasi</td>
                    <td className="border border-black p-3">Keterlibatan aktif murid, kesantunan adab, dan kemandirian dalam proyek.</td>
                  </tr>
                  <tr>
                    <td className="border border-black p-3 bg-gray-50/50 font-bold">Fokus Refleksi</td>
                    <td className="border border-black p-3">Merasakan kasih sayang antar teman dan kepuasan batin setelah melakukan kebaikan.</td>
                  </tr>
                  <tr>
                    <td className="border border-black p-3 bg-gray-50/50 font-bold">Output Diharapkan</td>
                    <td className="border border-black p-3 font-serif italic text-primary-800">"Menjadi hamba Allah yang bermanfaat bagi semesta dengan landasan cinta."</td>
                  </tr>
                </tbody>
              </table>
            </div>
  ...
  return (
    <div id="printable-module" className="bg-white dark:bg-neutral-900 p-8 md:p-12 shadow-lg border border-gray-200 dark:border-neutral-800 text-gray-900 dark:text-neutral-200 font-sans leading-relaxed max-w-[21cm] mx-auto mb-10 overflow-hidden transition-colors duration-300 print:shadow-none print:border-none print:p-0 relative">
      ...
      {pdfSections 
        ? pdfSections.filter(s => s.enabled).map((section, idx) => renderSection(section.id, idx))
        : ['informasiUmum', 'identifikasi', 'desainPembelajaran', 'deskripsiKegiatan', 'langkahKegiatan', 'asesmen', 'lampiran'].map((id, idx) => renderSection(id, idx))
      }
